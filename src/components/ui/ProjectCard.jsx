    ...
    <motion.div
      ...
      <div className="bg-emerald-50 dark:bg-gray-800/30 backdrop-blur-sm rounded-2xl overflow-hidden border transition-all duration-500 h-full flex flex-col
        ...
        <div className="relative h-fit mb-4">
          <ProjectCarousel images={carouselData} />
        </div>

        <div className="p-6 pt-4 flex flex-col flex-grow">
          <div className="flex items-center space-x-2">
            <h3 className="text-xl font-semibold text-slate-900 dark:text-white">{title}</h3>
            <div className={`inline-flex items-center justify-center ${categoryColor} text-xs font-semibold px-2 py-0.5 rounded-full shadow-inner dark:bg-none bg-slate-700`}>
              {category}
            </div>
          </div>

          <p className="mt-2 text-gray-900 dark:text-gray-300 flex-grow">{description}</p>

          <div className="mt-5 flex flex-wrap gap-2">
            {skills.map((skill, index) => (
              <span
                key={index}
                className={`px-2.5 py-1 text-xs rounded-full border ${skill.colorClasses} dark:bg-none bg-slate-600`}
              >
                {skill.name}
              </span>
            ))}
          ...
          <div className="mt-6 flex justify-between items-center">
            <div className="flex space-x-4">
              {liveLink && <Link
                ...
                className="flex items-center text-gray-900 dark:text-gray-300 hover:text-emerald-400 transition-colors duration-300"
